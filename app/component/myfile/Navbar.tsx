...
import { mdiViewDashboard, mdiMailboxOutline, mdiCog, mdiFileDownloadOutline, mdiNearMe, mdiInvoiceTextClockOutline, mdiListStatus, mdiMessageProcessingOutline } from '@mdi/js';
...
function Navbar({ status }: { status: string }) {
    return (
        <div className='text-white p-5 pt-8 w-64 flex flex-col justify-between  flex-shrink-0'>
            <div>
                <Link href="/dashboard" className='text-5xl text-center font-medium'>
                    Easy Doc
                </Link>
                <div className='mt-10'>
                    <Link href="dashboard" className={`${status === "dashboard" ? "bg-[#5852a5] border-s-4 border-white" : ""} hover:border-white hover:bg-[#5852a5] border-s-4 border-[#5955B3] flex cursor-pointer gap-3 py-3 px-4 `}>
